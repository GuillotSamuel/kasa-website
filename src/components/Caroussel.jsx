...
export default function Caroussel() {
  ...
  const carouselSettings = {
    dots: false,
    arrows: logementTrouve.pictures.length > 1,
    infinite: true,
    speed: 500,
    slidesToShow: 1,
    slidesToScroll: 1,
  ...
  return (
    <div id="carouselContainer">
      {logementTrouve.pictures.length > 1 && (
        <button className="carrouselButton previous" onClick={goToPreviousSlide}>
          <i className="fa-solid fa-angle-right fa-rotate-180 fa-2xl"></i>
        </button>
      )}
      {logementTrouve.pictures.length > 1 && (
        ...
        {logementTrouve.pictures.map((picture, index) => (
          <div key={index} className="logementSlide">
            <img
              src={picture}
              alt={`Image ${index + 1}`}
              className={`carrouselImage #${index + 1}`}
            />
          </div>
        ))}
      </Slider>
      {logementTrouve.pictures.length > 1 && (
        <div className="slideNumber">
          {currentSlide + 1}/{logementTrouve.pictures.length}
        </div>
      )}
    </div>
  );
